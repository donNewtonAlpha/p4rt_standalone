...
grpc::Status ValidateConnection(
    ...
    // If the election ID is not set then the controller is saying this should
    // be a backup connection, and we allow any number of backup connections.
    if (!election_id.has_value()) return grpc::Status::OK;

    // Otherwise, we verify the election ID is unique among all active
    // connections for a given role (including the root role).
...
absl::optional<uint64_t> SdnConnection::GetRoleId() const { return role_id_; }

void SdnConnection::SendStreamMessageResponse(
    const p4::v1::StreamMessageResponse &response) {
    if (!grpc_stream_->Write(response)) {
        LOG(ERROR)
            << "Could not send arbitration update response to gRPC conext '"
...
grpc::Status SdnControllerManager::HandleArbitrationUpdate(
    ...
    // If the controller is already initialized we check if the role & election
    // ID match. Assuming nothing has changed then there is nothing we need to
    // do.
    ...
    // Determine if there is any primary connection state changes. This check
    // will analize all the active connections so it needs to be done BEFORE we
    // update any connection state.
    return grpc::Status::OK;
}
...
bool SdnControllerManager::UpdateToPrimaryConnectionState(
    ...
    if (max_election_id != election_id_past || old_primary_is_reconnecting) {
        if (max_election_id.has_value() && max_election_id > election_id_past) {
            ...
            LOG(INFO)
                << "Old primary connection for role "
...
void SdnControllerManager::SendArbitrationResponse(SdnConnection *connection) {
    ...
    if (connection->GetRoleId().has_value()) {
        arbitration->mutable_role()->set_id(connection->GetRoleId().value());
    }
    ...
    if (PrimaryConnectionExists(connection->GetRoleId())) {
        ...
        if (primary_election_id == connection->GetElectionId()) {
            ...
            status->set_message(
                "you are a backup connection, and a primary connection "
                "exists.");
        }
    } else {
